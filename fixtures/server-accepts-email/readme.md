...
### `foobar(email[, baz]) => Future<bool>`

- `foo` (gsf, sf) - Email lorem ipsum
- `bar` (fsdect, fsd)
  - `baz` (afsad, fdsfd) - Domain to identify as (in `HELO` smtp command)
  - `qux` (da, sdgsd) - Email address to identify as (in `MAIL FROM` command)
